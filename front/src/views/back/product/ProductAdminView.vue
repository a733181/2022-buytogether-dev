...
const { banksAdmin } = storeToRefs(useBankStore());
...
const userbankList = computed(() => {
  return banksAdmin.value.list.filter(
    (item) => item.userId === editProduct.value.userId._id
  );
});

const newBankList = computed(() =>
  userbankList.value.map((item) => `${item.bankName} - ${item.bankNumber}`)
);

const editBank = computed(() => {
  const index = userbankList.value.findIndex(
    (item) => item._id === editProduct.value.bankId
  );
  if (index === -1) {
    return '';
  } else {
    return `${userbankList.value[index].bankName} - ${userbankList.value[index].bankNumber}`;
  }
});
...
const form = reactive({
  ...
  userId: editProduct.value.userId._id || null,
...
const validatorFormHandler = () => {
  ...
  const isBank = userbankList.value.some((item) => {
...
const submitHandler = async () => {
  ...
  const bankIndex = userbankList.value.findIndex((item) => {
    const bank = form.bank.split('-');
    return (
      item.bankName === bank[0].trim() &&
      item.bankNumber.toString() === bank[1].trim()
  ...
  fd.append('bankId', userbankList.value[bankIndex]._id);
